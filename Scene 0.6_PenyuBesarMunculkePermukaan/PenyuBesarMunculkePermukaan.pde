...
float shipX;
float shipY;
float shipSpeed = 1.5;  // Kecepatan pergerakan kapal
...
// Variabel untuk penyu
float turtleX;
float turtleY;  // Posisi penyu di bawah kapal
float turtleSpeed = 1.5;
...
void setup() {
  ...
  // Inisialisasi posisi penyu sejajar dengan ikan
  turtleX = shipX + 150;  // Posisi awal penyu di sebelah kapal
  turtleY = fishY[0];  // Posisi y penyu setara dengan ikan pertama
}
...
void draw() {
  ...
  // Menggambar penyu dan menggerakkannya
  drawTurtle(turtleX, turtleY, 0.75);  // Mengubah skala penyu menjadi 0.75
  turtleX += turtleSpeed;
  if (turtleX > width) {
    turtleX = -50; // Reset posisi penyu jika keluar dari layar
  }
}
...
// Fungsi untuk menggambar penyu
void drawTurtle(float x, float y, float scale) {
  float turtleWidth = 200 * scale;
  float turtleHeight = 150 * scale;
  float headSize = 50 * scale;
  float legSize = 40 * scale;
  
  fill(34, 139, 34);  // Warna hijau untuk cangkang penyu
  noStroke();
  
  // Cangkang penyu
  ellipse(x, y, turtleWidth, turtleHeight);

  // Kepala penyu
  ellipse(x + turtleWidth / 2, y, headSize, headSize);

  // Kaki penyu
  ellipse(x - turtleWidth / 3, y - turtleHeight / 2, legSize, legSize);
  ellipse(x - turtleWidth / 3, y + turtleHeight / 2, legSize, legSize);
  ellipse(x + turtleWidth / 3, y - turtleHeight / 2, legSize, legSize);
  ellipse(x + turtleWidth / 3, y + turtleHeight / 2, legSize, legSize);
  ...
  // Mata penyu
  fill(255);
  ellipse(x + turtleWidth / 2 + headSize / 4, y - headSize / 4, 10 * scale, 10 * scale);
  fill(0);
  ellipse(x + turtleWidth / 2 + headSize / 4, y - headSize / 4, 5 * scale, 5 * scale);
}
