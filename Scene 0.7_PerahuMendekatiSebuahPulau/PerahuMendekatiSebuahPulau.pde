...
void setup() {
  ...
  shipX = -200;  // posisi kapal di dalam layar
...
void drawIsland() {
  ...
  // Menggambar pohon kelapa di sekitar pulau
...
void drawShip(float x, float y, float scale) {
  ...
  MaleCharacter male = new MaleCharacter(x + shipWidth / 4, y - shipHeight, 0.8); // Ubah skala karakter menjadi lebih kecil
  male.drawCharacter();
  
  FemaleCharacter female = new FemaleCharacter(x + 3 * shipWidth / 4, y - shipHeight, 0.8); // Ubah skala karakter menjadi lebih kecil
...
void moveShip() {
  // Hentikan kapal jika mencapai pulau
  if (shipX + 200 * 1.0 > islandX && shipX < islandX + 400) {
    shipSpeed = 0;
  } else {
    shipSpeed = 1.7;
    shipX += shipSpeed;
